...
public class HudElementClockModern extends HudElementClockVanilla {
	...
	public boolean checkConditions() {
		return this.settings.getBoolValue(Settings.enable_clock) && !this.mc.gameSettings.showDebugInfo
				&& (!this.settings.getBoolValue(Settings.enable_immersive_clock) || this.mc.player.inventory.hasItemStack(new ItemStack(Items.CLOCK)));
	}
	...
	public void drawElement(AbstractGui gui, float zLevel, float partialTicks, int scaledWidth, int scaledHeight) {
		int yOffset = (this.settings.getBoolValue(Settings.render_player_face) ? 0 : 8)
				+ ((this.settings.getBoolValue(Settings.show_numbers_health)
						&& this.settings.getBoolValue(Settings.show_numbers_food)) ? 0 : 4)
