...
public class HudElementHealthDefault extends HudElement {
	...
	public void drawElement(AbstractGui gui, float zLevel, float partialTicks, int scaledHeight, int scaledWidth) {
		...
		int posY = (this.settings.getBoolValue(Settings.render_player_face) ? 13 : 5)
				+ this.settings.getPositionValue(Settings.health_position)[1];

		if (absorption > 1)
			drawCustomBar(posX, posY, 110, 12,
					(double) (health + absorption) / (double) (healthMax + absorption) * 100D, -1, -1,
					this.settings.getIntValue(Settings.color_absorption),
					offsetColorPercent(this.settings.getIntValue(Settings.color_absorption), OFFSET_PERCENT));

		if (this.mc.player.isPotionActive(Effects.POISON)) {
			drawCustomBar(posX, posY, 110, 12, (double) health / (double) (healthMax + absorption) * 100D, -1, -1,
