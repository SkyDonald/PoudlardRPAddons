...
public class HudElementHealthModern extends HudElement {
	...
	public void drawElement(AbstractGui gui, float zLevel, float partialTicks, int scaledHeight, int scaledWidth) {
		...
		if (this.settings.getBoolValue(Settings.show_numbers_health)
			...
			drawRect(textPosX + (this.settings.getBoolValue(Settings.render_player_face) ? 23 : 2), posY + 4, width, 8,
					0xA0000000);
			GL11.glScaled(0.5D, 0.5D, 0.5D);
			gui.drawCenteredString(this.mc.fontRenderer, stringHealth,
		...
		if (absorption > 1)
			drawTetragon(posX + 2, posX + 2, 5 + posY, 5 + posY,
					(int) (89 * ((double) (health + absorption) / (double) (healthMax + absorption))),
		...
		if (this.mc.player.isPotionActive(Effects.POISON)) {
			drawTetragon(posX + 2, posX + 2, 5 + posY, 5 + posY,
					(int) (89 * ((double) health / (double) (healthMax + absorption))),
					(int) (89 * ((double) health / (double) (healthMax + absorption))) - 10, 6, 6,
					this.settings.getIntValue(Settings.color_poison));
		} else if (this.mc.player.isPotionActive(Effects.WITHER)) {
			drawTetragon(posX + 2, posX + 2, 5 + posY, 5 + posY,
					(int) (89 * ((double) health / (double) (healthMax + absorption))),
					(int) (89 * ((double) health / (double) (healthMax + absorption))) - 10, 6, 6,
					this.settings.getIntValue(Settings.color_wither));
		} else {
			drawTetragon(posX + 2, posX + 2, 5 + posY, 5 + posY,
					(int) (89 * ((double) health / (double) (healthMax + absorption))),
					(int) (89 * ((double) health / (double) (healthMax + absorption))) - 10, 6, 6,
