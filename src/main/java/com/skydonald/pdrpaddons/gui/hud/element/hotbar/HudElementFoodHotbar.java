	...
	public void drawElement(AbstractGui gui, float zLevel, float partialTicks, int scaledWidth, int scaledHeight) {
		...
		if (stats.needFood() && this.settings.getBoolValue(Settings.show_hunger_preview)) {
			...
			if (value > 0) {
				...
				drawCustomBar(posX, height - 26, 200, 10, bonusHunger / (double) staminaMax * 100.0D, -1, -1,
						colorPreview, offsetColorPercent(colorPreview, OFFSET_PERCENT));
			}
		}

		if (this.mc.player.isPotionActive(Effects.HUNGER)) {
			drawCustomBar(posX, height - 26, 200, 10, stamina / (double) staminaMax * 100.0D, -1, -1,
		...
		String staminaString = this.settings.getBoolValue(Settings.hunger_percentage) == true
