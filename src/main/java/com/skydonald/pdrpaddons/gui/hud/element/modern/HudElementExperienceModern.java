...
public class HudElementExperienceModern extends HudElement {
	...
	public void drawElement(AbstractGui gui, float zLevel, float partialTicks, int scaledWidth, int scaledHeight) {
		...
		drawRect(1 + posX, scaledHeight - 6 + posY, (int) (exp * full), 4,
				this.settings.getIntValue(Settings.color_experience));
		...
		if (this.settings.getBoolValue(Settings.show_numbers_experience)) {
			...
			gui.drawCenteredString(this.mc.fontRenderer, stringExp, 6 + width2 + posX * 2,
					(scaledHeight - 12) * 2 - 1 + posY * 2, -1);
