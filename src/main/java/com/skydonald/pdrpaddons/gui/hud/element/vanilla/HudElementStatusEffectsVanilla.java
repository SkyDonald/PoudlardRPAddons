...
public class HudElementStatusEffectsVanilla extends HudElement {
    ...
    @Override
    public void drawElement(AbstractGui gui, float na, float partialTicks, int scaledWidth, int scaledHeight) {
            ...
            for (EffectInstance effectinstance : Ordering.natural().reverse().sortedCopy(collection)) {
                ...
                if (effectinstance.isShowIcon()) {
                    ...
                    if (effect.isBeneficial()) {
                        ++i;
                        if (main.settings.getBoolValue(Settings.status_vertical)) {
                            k -= 25;
                            l += 25 * (i - 1);
                        } else {
                            k -= 25 * i;
                    ...
                    AbstractGui.blit(k + 3, l + 3, gui.getBlitOffset(), 18, 18, textureatlassprite);
                    // Main
                    effectinstance.renderHUDEffect(gui, k, l, gui.getBlitOffset(), f);
                    if (main.settings.getBoolValue(Settings.status_time) && !effectinstance.isAmbient()) {
                        int duration = effectinstance.getDuration() / 20;
                        String s = "*:**";
                        if (duration < 600)
                            s = String.valueOf(duration / 60 + ":"
