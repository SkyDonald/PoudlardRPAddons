	...
	private int calculateWidth() {
		...
		if (item != ItemStack.EMPTY) {
			if (this.settings.getBoolValue(Settings.show_item_durability) && item.isDamageable()) {
				...
				int z = 0;
				if (PoudlardRPAddons.renderDetailsAgain[0]
					...
					for (int y = 0; y < x; y++) {
						...
						if (item != ItemStack.EMPTY && Item.getIdFromItem(item.getItem()) == Item
								.getIdFromItem(this.mc.player.getHeldItemMainhand().getItem())) {
							z += item.getCount();
						}
					}
					this.count1 = z;
		...
		if (item != ItemStack.EMPTY) {
			if (this.settings.getBoolValue(Settings.show_item_durability) && item.isDamageable()) {
				...
				int x = this.mc.player.inventory.getSizeInventory();
				int z = 0;
				if (PoudlardRPAddons.renderDetailsAgain[1] || !ItemStack.areItemStacksEqual(this.itemOffhandLast, item)
						|| !ItemStack.areItemStacksEqual(this.itemMainHandLast, item)) {
					...
					for (int y = 0; y < x; y++) {
						item = this.mc.player.inventory.getStackInSlot(y);
						if (item != ItemStack.EMPTY && Item.getIdFromItem(item.getItem()) == Item
								.getIdFromItem(this.mc.player.getHeldItemOffhand().getItem())) {
							z += item.getCount();
						}
					}
					this.count2 = z;
		...
		if (this.settings.getBoolValue(Settings.show_arrow_count) && item != ItemStack.EMPTY
				&& this.mc.player.getHeldItemMainhand().getItem() instanceof BowItem) {
			int x = this.mc.player.inventory.getSizeInventory();
			int z = 0;
	...
	protected void drawItemDetails(AbstractGui gui, Hand hand, int width) {
		...
		if (item != ItemStack.EMPTY) {
			if (this.settings.getBoolValue(Settings.show_item_durability) && item.isDamageable()) {
				...
				int x = this.mc.player.inventory.getSizeInventory();
				int z = 0;
				if ((hand == Hand.MAIN_HAND ? PoudlardRPAddons.renderDetailsAgain[0]
						: PoudlardRPAddons.renderDetailsAgain[1])
					...
					for (int y = 0; y < x; y++) {
						item = this.mc.player.inventory.getStackInSlot(y);
						if (item != ItemStack.EMPTY && Item.getIdFromItem(item.getItem()) == Item
								.getIdFromItem(this.mc.player.getHeldItem(hand).getItem())) {
							z += item.getCount();
						}
					}
					if (hand == Hand.MAIN_HAND)
