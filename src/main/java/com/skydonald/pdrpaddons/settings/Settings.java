...
public class Settings {
    ...
    private File pdrpDir() {
        Minecraft mc = Minecraft.getInstance();
        return (new File(mc.gameDir.getPath(), "config" + File.separator + "PDRP.cfg"));
    }
    ...
    public void init() {
        ...
        addSetting(color_absorption,
                new SettingColor(color_absorption, HudElementType.HEALTH, HudElement.COLOR_ORANGE));
        ...
        addSetting(show_numbers_experience,
                new SettingBoolean(show_numbers_experience, HudElementType.EXPERIENCE, true));
        addSetting(experience_percentage, new SettingBoolean(experience_percentage, HudElementType.EXPERIENCE, false));
        addSetting(color_experience,
                new SettingColor(color_experience, HudElementType.EXPERIENCE, HudElement.COLOR_YELLOW));
        addSetting(experience_position, new SettingPosition(experience_position, HudElementType.EXPERIENCE, 0, 0));

        addSetting(mount_health_percentage,
                new SettingBoolean(mount_health_percentage, HudElementType.HEALTH_MOUNT, false));
        ...
        addSetting(clock_time_format,
                new SettingString(clock_time_format, HudElementType.CLOCK, 0, new String[] { "time.24", "time.12" }));
        ...
        addSetting(enable_immersive_compass,
                new SettingBoolean(enable_immersive_compass, HudElementType.COMPASS, false));
        addSetting(enable_compass_coordinates,
                new SettingBoolean(enable_compass_coordinates, HudElementType.COMPASS, true));
        ...
        addSetting(enable_entity_inspect,
                new SettingBoolean(enable_entity_inspect, HudElementType.ENTITY_INSPECT, true));
        ...
        addSetting(mount_health_position,
                new SettingPosition(mount_health_position, HudElementType.HEALTH_MOUNT, 0, 0));
    ...
    public void addDebugSettings(HudElementType type) {
        ...
        // false));
        addSetting(render_vanilla + "_" + type.name().toLowerCase(),
                new SettingBooleanDebug(render_vanilla + "_" + type.name().toLowerCase(), type, false));
        addSetting(prevent_event + "_" + type.name().toLowerCase(),
                new SettingBooleanDebug(prevent_event + "_" + type.name().toLowerCase(), type, false));
    ...
    public String getButtonString(String id) {
        ...
        if (setting instanceof SettingBoolean) {
            return s + (setting.getBoolValue() ? I18n.format("options.on")
            ...
            return s + (Objects.equals(id, pickup_duration)
                    ? Math.ceil(SettingFloat.snapToStepClamp(sf, sf.getFloatValue())) + " "
                            + I18n.format("gui.pdrp.sec")
                ...
                while (true) {
                    ...
                    if (line.contains(":") && line.contains("=")) {
                        ...
                        if (this.getSetting(setting[0]) != null) {
                            if (type[0].matches("B")) {
                                this.setSetting(setting[0],
                                        this.getSetting(setting[0]).setValue(Boolean.valueOf(setting[1])));
                            } else if (type[0].matches("S")) {
                                this.setSetting(setting[0], this.getSetting(setting[0]).setValue(setting[1]));
                            } else if (type[0].matches("C")) {
                                this.setSetting(setting[0],
                                        this.getSetting(setting[0]).setValue(Integer.valueOf(setting[1])));
                                ...
                                this.setSetting(setting[0],
                                        this.getSetting(setting[0]).setValue(Float.valueOf(setting[1])));
                            } else if (type[0].matches("D")) {
                                this.setSetting(setting[0],
                                        this.getSetting(setting[0]).setValue(Double.valueOf(setting[1])));
