...
public class SettingDouble extends Setting {
	...
	public SettingDouble(String ID, HudElementType type, double defaultValue, double minValue, double maxValue,
			double step) {
		super(ID, type);
		this.defaultValue = defaultValue;
		this.value = defaultValue;
	...
	public static double normalizeValue(SettingDouble setting, double value) {
		return MathHelper.clamp(
				(snapToStepClamp(setting, value) - setting.minValue) / (setting.maxValue - setting.minValue), 0.0F,
				1.0F);
	}

	public static double denormalizeValue(SettingDouble setting, double value) {
		return snapToStepClamp(setting,
				setting.minValue + (setting.maxValue - setting.minValue) * MathHelper.clamp(value, 0.0F, 1.0F));
	}
